...
func Start(args []string) {
	...
	serv_args := []string{binpath + processus, "-host", host, "-port", port}

	// Lancer le processus en arrière-plan
	proc, err := os.StartProcess(binpath+processus, serv_args, attr)
	if err != nil {
	...
	err = os.WriteFile(PID_FILE, []byte(strconv.Itoa(proc.Pid)), 0644)
	if err != nil {
		log.Fatalf("Erreur lors de l'écriture du fichier PID : %v\n", err)
	}

	runtimeArgs := []string{"-host", host, "-port", port}
	cmd := exec.Command(binpath+processus, runtimeArgs...)
	err = cmd.Start()
	if err != nil {
		log.Fatalln(err)
	}
...
// func ask_user_for_port() {
// 	print(Get_prompt() + "Enter the \033[36mPORT\033[0m on which the server will listen : ")
// 	var input string
// 	_, _ = fmt.Scanln(&input)
// 	Set_port(input)
// }
